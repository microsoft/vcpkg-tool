...
'@ | % { $_ -replace "`r","" } | Out-File -enc ascii $VcpkgToolsJson
    ...
    $env:VCPKG_DOWNLOADS = Join-Path $TestingRoot 'system tool downloads'
    $env:VCPKG_FORCE_SYSTEM_BINARIES = "1"
    $env:VCPKG_TEST_NINJA_VERSION = "0.0.0"
    ...
    $env:VCPKG_TEST_NINJA_VERSION = "1.9.0"
    Run-Vcpkg -TestArgs ($commonArgs + @("fetch", "ninja", "--vcpkg-root=$TestingRoot"))
    Throw-IfFailed
    Require-FileNotExists "$env:VCPKG_DOWNLOADS/tools/ninja-1.10.2-windows/ninja.exe"

    # Without min-version, a system copy matching the downloadable version must be accepted.
    $toolDataWithoutMinVersion = (Get-Content -Raw $VcpkgToolsJson) -replace '(?m)^\s*"min-version": "1\.9\.0",\r?\n', ''
    $toolDataWithoutMinVersion | Out-File -Encoding ascii $VcpkgToolsJson
    $env:VCPKG_DOWNLOADS = Join-Path $TestingRoot 'version fallback downloads'
    $env:VCPKG_TEST_NINJA_VERSION = "1.10.2"
    Run-Vcpkg -TestArgs ($commonArgs + @("fetch", "ninja", "--vcpkg-root=$TestingRoot"))
    Throw-IfFailed
    Require-FileNotExists "$env:VCPKG_DOWNLOADS/tools/ninja-1.10.2-windows/ninja.exe"
    ...
    Remove-Item env:VCPKG_TEST_NINJA_VERSION

    # An exact-version lookup must compare against the downloaded version, not min-version.
    # Otherwise every lookup extracts the archive again because CMake reports 3.22.2 rather than 3.17.1.
    $env:VCPKG_DOWNLOADS = Join-Path $TestingRoot 'exact version downloads'
    $env:VCPKG_FORCE_DOWNLOADED_BINARIES = "1"
    $exactCMakeArgs = $commonArgs + @(
        "fetch",
        "cmake",
        "--vcpkg-root=$TestingRoot",
    ...
    $secondExactFetchOutput = Run-VcpkgAndCaptureOutput -TestArgs $exactCMakeArgs
    Throw-IfFailed
    if ($secondExactFetchOutput -match "Extracting cmake") {
        throw "Exact-version fetch extracted an already-installed CMake archive"
    }
