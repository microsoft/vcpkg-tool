...
if ($Filter -ne $null) {
    $AllTests = $AllTests | Where-Object { $_.Name -match $Filter }
}
...
for ($n = 1; $n -le $allTestsCount; $n++)
{
    ...
    if ($lastTextExitCode -ne 0)
    {
        Write-Host -ForegroundColor Red "[end-to-end-tests.ps1] Suite $testDisplayName failed with exit code $lastTextExitCode"
