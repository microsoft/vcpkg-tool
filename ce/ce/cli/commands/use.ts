...
export class UseCommand extends Command {
  ...
  override async run() {
    ...
    const success = await activate(artifacts, registries, false, { force: this.commandLine.force, language: this.commandLine.language, allLanguages: this.commandLine.allLanguages });
    if (success) {
