...
describe('Amf', () => {
  ...
  it('parses acquisition-tags', async() => {
    const content = JSON.stringify({
      'id': 'example',
      'version': '1.0.0',
      'acquisition-tags': ['a', 'b']
    });

    const doc = await MetadataFile.parseConfiguration('acquisition-tags-test', content, local.session);
    const validationErrors = Array.from(doc.validate(), (error) => doc.formatVMessage(error));
    strict.sequenceEqual(validationErrors, []);
    strict.sequenceEqual(doc.acquisitionTags, ['a', 'b']);
  });

  it('requires acquisition-tags to be an array', async() => {
    const content = JSON.stringify({
      'id': 'example',
      'version': '1.0.0',
      'acquisition-tags': 'a'
    ...
    const validationErrors = Array.from(doc.validate(), (error) => doc.formatVMessage(error));
    strict.sequenceEqual(validationErrors, [
      'acquisition-tags-test:1:54 IncorrectType, acquisition-tags should be a sequence, found \'string\'']);
  });

  it('requires acquisition-tags to be letters (single)', async() => {
    const content = JSON.stringify({
      'id': 'example',
      'version': '1.0.0',
      'acquisition-tags': ['a-']
    ...
    const validationErrors = Array.from(doc.validate(), (error) => doc.formatVMessage(error));
    strict.sequenceEqual(validationErrors, [
      'acquisition-tags-test:1:54 ParseError, acquisition-tags may only be letters, found bad tags a-']);
  });

  it('requires acquisition-tags to be letters (multi)', async() => {
    const content = JSON.stringify({
      'id': 'example',
      'version': '1.0.0',
      'acquisition-tags': ['a-', 'b-', 'see']
    });

    const doc = await MetadataFile.parseConfiguration('acquisition-tags-test', content, local.session);
    const validationErrors = Array.from(doc.validate(), (error) => doc.formatVMessage(error));
    strict.sequenceEqual(validationErrors, [
      'acquisition-tags-test:1:54 ParseError, acquisition-tags may only be letters, found bad tags a-,b-,see']);
  });
});
