...
namespace vcpkg::Hash
{
    ...
    inline std::string get_file_hash(LineInfo li, const Filesystem& fs, const path& path, Algorithm algo) noexcept
    {
        std::error_code ec;
        ...
        if (ec)
        {
            Checks::exit_with_message(
                li, "Failure to read file '%s' for hashing: %s", vcpkg::u8string(path), ec.message());
        }
