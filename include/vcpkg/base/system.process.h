...
namespace vcpkg
{
    struct CMakeVariable
    {
        ...
        CMakeVariable(const StringView varname, const stdfs::path& path);
    ...
    {
        Command() = default;
        explicit Command(const stdfs::path& p) { path_arg(p); }
        ...
        Command& path_arg(const stdfs::path& p) & { return string_arg(vcpkg::Files::u8string(p)); }
        ...
        Command&& path_arg(const stdfs::path& p) && { return std::move(path_arg(p)); }
    ...
    {
        const stdfs::path& working_directory;
    };
    ...
    {
        return cmd_execute(cmd_line, InWorkingDirectory{stdfs::path()}, env);
    }

    int cmd_execute_clean(const Command& cmd_line, InWorkingDirectory wd);
    inline int cmd_execute_clean(const Command& cmd_line)
    {
        return cmd_execute_clean(cmd_line, InWorkingDirectory{stdfs::path()});
    }
    ...
    {
        return cmd_execute_and_capture_output(cmd_line, InWorkingDirectory{stdfs::path()}, env);
    }
    ...
    inline int cmd_execute_and_stream_lines(const Command& cmd_line,
                                            ...
                                            const Environment& env = {})
    {
        return cmd_execute_and_stream_lines(cmd_line, InWorkingDirectory{stdfs::path()}, std::move(per_line_cb), env);
    }
    ...
    inline int cmd_execute_and_stream_data(const Command& cmd_line,
                                           ...
                                           const Environment& env = {})
    {
        return cmd_execute_and_stream_data(cmd_line, InWorkingDirectory{stdfs::path()}, std::move(data_cb), env);
    }
