...
template <>
struct fmt::formatter<vcpkg::PackageSpec>
{
    ...
    auto format(const vcpkg::PackageSpec& spec, FormatContext& ctx) -> decltype(ctx.out())
    {
        return format_to(ctx.out(), "{}", spec.to_string());
    }
};
...
template<>
struct std::equal_to<vcpkg::FeatureSpec>
{
