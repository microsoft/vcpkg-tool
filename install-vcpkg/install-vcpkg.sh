...
VCPKG_bootstrap_node() {
  ...
  if [ ! -z "$N" ]; then
    if [ -f $N ]; then
      if [ `$N -e "[major, minor, patch ] = process.versions.node.split('.'); console.log( !!(major>16 || major == 16 & minor >= 12) )"` = "true" ]; then
        VCPKG_NODE=$N
        VCPKG_NPM=`which npm`
        VCPKG_debug using node in path
        return 0
      fi
    fi
  fi
  ...
  if [ -f $VCPKG_NODE ]; then
    if [ `$VCPKG_NODE -e "[major, minor, patch ] = process.versions.node.split('.'); console.log( !!(major>16 || major == 16 & minor >= 12))"` = "true" ]; then
      # looks good, lets grab NPM in case we need it, and get out.
      VCPKG_NPM=`which ${CE}/bin/npm`
      VCPKG_debug using node from ce
      return 0
    fi
  fi
  ...
  local VCPKG_ARCHIVE="${CE}/files/${NODE_FULLNAME}${VCPKG_ARCHIVE_EXT}"

  if [ ! -d "${CE}/files" ]; then
    command mkdir -p "${CE}/files"
  fi
  ...
  if [ "${VCPKG_OS}" = "aix" ]; then
    gunzip "${VCPKG_ARCHIVE}" | tar -xvC "${CE}" "${NODE_FULLNAME}/bin/${NODE_EXE}" >> $VCPKG_ROOT/log.txt 2>&1
  else
    tar $TAR_FLAGS "${VCPKG_ARCHIVE}" -C "${CE}" >> $VCPKG_ROOT/log.txt 2>&1
  fi
  mv "${CE}/${NODE_FULLNAME}/bin" "${CE}/" >> $VCPKG_ROOT/log.txt 2>&1
  mv "${CE}/${NODE_FULLNAME}/lib" "${CE}/" >> $VCPKG_ROOT/log.txt 2>&1
  rm -r "${CE}/${NODE_FULLNAME}" >> $VCPKG_ROOT/log.txt 2>&1
  ...
  if [ ! -f $VCPKG_NODE ]; then
    ...
    if [ "$VCPKG_OS" = "freebsd" ]; then
      ...
      echo ""
      echo "run (as root): "
      echo "   pkg install -y node16"
      echo "   pkg install -y npm-node16"
    fi
    return 1;
  fi
...
# go back where we were
  ...
  cp $CE/node_modules/.bin/* $VCPKG_ROOT/
...
# is ce installed?
if [ $? -eq 1 ]; then
  VCPKG_debug failed to bootstrap ce.js
  VCPKG_cleanup
  return 1;
fi
