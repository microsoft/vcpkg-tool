...
TEST_CASE ("generate_nuspec", "[generate_nuspec]")
{
    ...
    auto pkgPath =
        fsWrapper.absolute(VCPKG_LINE_INFO, vcpkg::Files::u8path("/zlib2_x64-windows")) / vcpkg::Files::u8path("**");
    pkgPath.make_preferred();
    const auto pkgPathStr = vcpkg::Files::u8string(pkgPath);
...
Build-Depends: bzip
    ...
    auto maybe_scf =
        SourceControlFile::parse_control_file(vcpkg::Files::u8string(stdfs::path()), std::move(*pghs.get()));
    REQUIRE(maybe_scf.has_value());
    SourceControlFileLocation scfl{std::move(*maybe_scf.get()), stdfs::path()};
...
Description: a spiffy compression library wrapper
    ...
    auto maybe_scf2 =
        SourceControlFile::parse_control_file(vcpkg::Files::u8string(stdfs::path()), std::move(*pghs2.get()));
    REQUIRE(maybe_scf2.has_value());
    SourceControlFileLocation scfl2{std::move(*maybe_scf2.get()), stdfs::path()};
    plan.install_actions.push_back(Dependencies::InstallPlanAction());
