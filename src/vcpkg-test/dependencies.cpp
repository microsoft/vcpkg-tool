...
struct MockVersionedPortfileProvider : PortFileProvider::IVersionedPortfileProvider
{
                                       ...
                                       Versions::Scheme scheme = Versions::Scheme::String)
    {
        ...
        if (it2 == it->second.end())
        {
            ...
            it2 = it->second.emplace(version, SourceControlFileLocation{std::move(scf), vcpkg::Files::u8path(name)})
                      .first;
        }
        return it2->second;
    }
...
struct MockOverlayProvider : PortFileProvider::IOverlayProvider
{
                                       ...
                                       Versions::Scheme scheme = Versions::Scheme::String)
    {
        ...
        if (it == mappings.end())
        {
            ...
            core->version_scheme = scheme;
            scf->core_paragraph = std::move(core);
            it = mappings.emplace(name, SourceControlFileLocation{std::move(scf), vcpkg::Files::u8path(name)}).first;
        }
        return it->second;
    }
