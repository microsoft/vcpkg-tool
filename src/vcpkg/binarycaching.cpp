    ...
    struct GHABinaryProvider : IBinaryProvider
    {
        ...
        void push_success(const InstallPlanAction& action) const override
        {
            ...
            if (!m_write_url.empty())
            {
                ...
                {
                    msg::write_unlocalized_text_to_stdout(Color::none, fmt::format("Cache Id: ", cacheId.value_or("no value")));
    ...
    const VcpkgPaths& paths, View<std::string> args)
{
    ...
    {
        auto url_optional = get_environment_variable("ACTIONS_CACHE_URL");
        ...
        providers.push_back(std::make_unique<GHABinaryProvider>(paths, s.gha_read, s.gha_write, url, token_header));
    }
