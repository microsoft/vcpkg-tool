    ...
    struct ZipReadBinaryProvider : IReadBinaryProvider
    {
        ...
        void fetch(View<const InstallPlanAction*> actions, Span<RestoreResult> out_status) const override
        {
            ...
            for (size_t i = 0; i < actions.size(); ++i)
            {
                ...
                clean_prepare_dir(m_fs, actions[i]->package_dir);
                jobs.push_back(m_zip.decompress_zip_archive_cmd(actions[i]->package_dir, zip_paths[i].get()->path));
...
namespace vcpkg
{
    ...
    void BinaryCache::push_success(const InstallPlanAction& action)
    {
        ...
        {
            m_fs.remove_all(action.package_dir, IgnoreErrors{});
        }
    }
                                   ...
                                   const NuGetRepoInfo& rinfo)
{
    ...
    const auto& pgh = *scf.core_paragraph;
    const auto& abi_info = action.abi_info;
