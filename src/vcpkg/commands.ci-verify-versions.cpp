        ...
        if (verify_git_trees)
        {
            for (auto&& version_entry : *versions)
            {
                ...
                for (StringView control_file : {"CONTROL", "vcpkg.json"})
                {
                    ...
                    auto maybe_scfl =
                        control_file == "vcpkg.json"
                            ? Paragraphs::try_load_port_manifest_text(file, treeish, StringView{}, stdout_sink)
                            : Paragraphs::try_load_control_file_text(file, treeish, StringView{});
