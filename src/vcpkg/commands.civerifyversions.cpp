...
namespace vcpkg::Commands::CIVerifyVersions
{
    ...
    static ExpectedS<std::string> verify_version_in_db(const VcpkgPaths& paths,
                                                       ...
                                                       const stdfs::path& port_path,
                                                       const stdfs::path& versions_file_path,
        ...
        if (verify_git_trees)
        {
            for (auto&& version_entry : versions)
            {
                ...
                for (const std::string& control_file : {"CONTROL", "vcpkg.json"})
                {
                    ...
                    auto maybe_file =
                        paths.git_show(Strings::concat(treeish), paths.root / vcpkg::Files::u8path(".git"));
    ...
    void perform_and_exit(const VcpkgCmdArguments& args, const VcpkgPaths& paths)
    {
        ...
        for (const auto& dir : stdfs::directory_iterator(paths.builtin_ports_directory()))
        {
            ...
            auto&& port_name = vcpkg::Files::u8string(port_path.stem());
            ...
            if (git_tree_it == port_git_tree_map.end())
            {
                ...
                errors.emplace(Strings::format("Error: While validating port %s.\n"
                                               ...
                                               port_name,
                                               vcpkg::Files::u8string(port_path),
                                               port_name,
                                               port_name));
                continue;
            }
            ...
            auto control_path = port_path / vcpkg::Files::u8path("CONTROL");
            auto manifest_path = port_path / vcpkg::Files::u8path("vcpkg.json");
            ...
            auto versions_file_path = paths.builtin_registry_versions / vcpkg::Files::u8path({port_name[0], '-'}) /
