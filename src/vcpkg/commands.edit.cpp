...
namespace vcpkg
{
    ...
    void command_edit_and_exit(const VcpkgCmdArguments& args, const VcpkgPaths& paths)
    {
        ...
        if (succeeded(cmd_execute({Command("command").string_arg("-v").string_arg("xdg-mime")})))
        {
            ...
            {
                mime_qry.cmd = Command("command").string_arg("-v").string_arg(output->substr(0, output->find('.')));
        ...
        ProcessLaunchSettings settings{Command(env_editor).raw_arg(args_as_string).string_arg("-n")};
#if defined(_WIN32)
        ...
        if (editor_exe == "Code.exe" || editor_exe == "Code - Insiders.exe")
        {
            ...
            cmd_execute_background(Command("cmd").string_arg("/d").string_arg("/c").raw_arg(
                Strings::concat('"', settings.command_line(), R"( <NUL")")));
            Checks::exit_success(VCPKG_LINE_INFO);
        }
#endif // ^^^ _WIN32

        Checks::exit_with_code(VCPKG_LINE_INFO, cmd_execute(settings).value_or_exit(VCPKG_LINE_INFO));
    }
} // namespace vcpkg
