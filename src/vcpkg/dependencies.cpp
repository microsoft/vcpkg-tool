...
namespace vcpkg
{
    ...
    FormattedPlan format_plan(const ActionPlan& action_plan, const Path& builtin_ports_dir)
    {
        ...
        if (!excluded.empty())
        {
            format_plan_block(ret.warning_text, msgExcludedPackages, false, excluded, builtin_ports_dir);
        }

        if (!already_installed_head_plans.empty())
        {
            format_plan_block(
                ret.warning_text, msgInstalledPackagesHead, false, already_installed_head_plans, builtin_ports_dir);
        }

        if (!already_installed_plans.empty())
        {
            format_plan_block(ret.normal_text, msgInstalledPackages, false, already_installed_plans, builtin_ports_dir);
        }
        ...
        {
            format_plan_block(ret.normal_text, msgPackagesToRebuild, true, rebuilt_plans, builtin_ports_dir);
        }

        if (!new_plans.empty())
        {
            format_plan_block(ret.normal_text, msgPackagesToInstall, true, new_plans, builtin_ports_dir);
        }
