...
namespace vcpkg
{
                                   ...
                                   Triplet host_triplet)
    {
        ...
        LockGuardPtr<Metrics>(g_metrics)->track_property(BoolMetric::InstallManifestMode,
        ...
        if (auto manifest = paths.get_manifest().get())
        {
            ...
            {
                LockGuardPtr<Metrics>(g_metrics)->track_property(DefineMetric::X_WriteNugetPackagesConfig);
            ...
            {
                LockGuardPtr<Metrics>(g_metrics)->track_property(DefineMetric::ManifestVersionConstraint);
            }

            if (!manifest_core.overrides.empty())
            {
                LockGuardPtr<Metrics>(g_metrics)->track_property(DefineMetric::ManifestOverrides);
            }
    ...
    void track_install_plan(ActionPlan& plan)
    {
        ...
        LockGuardPtr<Metrics>(g_metrics)->track_property(StringMetric::InstallPlan_1, specs_string);
    }
}
