...
#undef OUTDATED_V_NO_120
                                             ...
                                             MessageSink& msg_sink)
    {
        ...
        else if (cmake_system_name == "Darwin")
        {
            ...
            for (const Path& file : files)
            {
                RedirectedProcessLaunchSettings settings{Command("lipo").string_arg("-archs").string_arg(file)};
                auto maybe_output = flatten_out(cmd_execute_and_capture_output(settings), "lipo");
