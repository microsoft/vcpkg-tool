    ...
    private:
        ...
        Path get_versions_tree_path() const
        {
            return m_versions_tree.get([this]() -> Path {
                ...
                if (!maybe_tree)
                {
                    LockGuardPtr<Metrics>(g_metrics)->track_property(DefineMetric::RegistriesErrorNoVersionsAtCommit);
    ...
    ExpectedL<Version> GitRegistry::get_baseline_version(StringView port_name) const
    {
        const auto& baseline = m_baseline.get([this]() -> Baseline {
            ...
            if (!maybe_contents)
            {
                ...
                if (!maybe_err)
                {
                    LockGuardPtr<Metrics>(g_metrics)->track_property(DefineMetric::RegistriesErrorCouldNotFindBaseline);
                    Checks::exit_with_message(
                        VCPKG_LINE_INFO,
