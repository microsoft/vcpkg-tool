                                                                         ...
                                                                         bool is_default_builtin_registry) const
    {
        ...
        if (!flags.versions)
        {
            auto check_deps = [&](View<Dependency> deps) -> Optional<std::string> {
                for (auto&& dep : deps)
                {
                    ...
                    {
                        get_global_metrics_collector().track_define_property(DefineMetric::ErrorVersioningDisabled);
        ...
        else
        {
            if (!core_paragraph->builtin_baseline.has_value() && is_default_builtin_registry)
            {
                ...
                {
                    get_global_metrics_collector().track_define_property(DefineMetric::ErrorVersioningNoBaseline);
                    return Strings::concat(
                        origin,
