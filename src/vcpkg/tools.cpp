...
namespace vcpkg
{
    ...
    // The CMake and PowerShell versions are included in the ABI info.
    // Allowing use of system copies of these tools above a specified minimum
    // version would make it cumbersome to reuse binary caches generated by
    // servers for local development. If the server had a newer version of the tool,
    // developers building locally would have install the exact same version of
    // the tools and keep them updated whenever the server is updated. This is
    // especially problematic when using servers with environments not
    // controlled by the development team, for example GitHub Actions.
    // Refer to discussion on https://github.com/microsoft/vcpkg/issues/16615
    // for context.
    ...
    struct ToolProvider
    {
        ...
        virtual VersionRequirement get_version_requirement() const { return VersionRequirement::MinimumOrGreater; };
    };
                                                                       ...
                                                                       const std::array<int, 3>& expected_version)
    {
        ...
        for (auto&& candidate : candidates)
        {
            ...
            bool version_acceptable = false;
            switch (tool_provider.get_version_requirement())
            {
                case VersionRequirement::ExactMatchOnly:
                    version_acceptable = actual_version[0] == expected_version[0] &&
                                         actual_version[1] == expected_version[1] &&
                                         actual_version[2] == actual_version[2];
                    break;
                case VersionRequirement::MinimumOrGreater:
                default:
                    version_acceptable =
...
CMake suite maintained and supported by Kitware (kitware.com/cmake).
        ...
        virtual VersionRequirement get_version_requirement() const override
        {
            return VersionRequirement::ExactMatchOnly;
        }
    };
