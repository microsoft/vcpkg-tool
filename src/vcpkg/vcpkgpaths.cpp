...
namespace vcpkg
{
		...
		const Optional<Json::Object>& configuration_from_manifest)
	{
		...
		if (!fs.exists(path_to_config, IgnoreErrors{}))
		{
			...
			return { std::move(config_dir), deserialize_configuration(configuration_from_manifest.value_or_exit(VCPKG_LINE_INFO), args, manifest_dir / "vcpkg.json") };
		}
		...
		if (configuration_from_manifest.has_value())
		{
			print2(Color::error,
				...
				"-- Or remove \"$x-vcpkg-configuration\" from manifest file \"", manifest_dir / "vcpkg.json", "\".");
			Checks::exit_fail(VCPKG_LINE_INFO);
		}
